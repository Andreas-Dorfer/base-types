...
let main argv =
    ...
    match "abc" |> BaseType.from<MyMaxLengthString, _> with
    ...
    let someWeekend = someWeekenInThe90s |> BaseType.mapValue BaseType.from<SomeWeekend, _>

    let x =
        (1995, 1, 1)
        |> DateTime
        |> BaseType.from<SomeWeekendInThe90s, _>
